...
def main(args):
    ...
    test_files = os.listdir(args.files)
    #print('files', args.files, 'test_file', test_files)

    
    for img_name in test_files:
        start = time.time()
        img_fp = args.files+img_name
        img_ori = cv2.imread(img_fp)
        if args.dlib_bbox:
            rects = face_detector(img_ori, 1)
        else:
            rects = []
        # 얼굴 검출 못할 경우 다음 이미지로 넘어가기 
        if len(rects) == 0:
            continue
       ...
       # print('time', time.time() - start)
        ...
        # detect시간 text로 저장
        save_detect_time = "/home/teamnova/3DDFA/facebook/result.text"
        dirname = os.path.dirname(save_detect_time)
    #    if not os.path.isdir(dirname):
    #       os.makedirs(dirname)
        with open(save_detect_time, "a") as fd:
            line = img_fp + " im_detect: " + str(time.time() - start)  + " \n"
            fd.write(line)
        #print('time', time.time() - start)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--files', help='image files paths fed into network, single or multiple images')
